...
Файл `ad_fetcher_stub.py` позволяет тестировать приложение без подключения к корпоративному AD.

**Включение** — добавить в `config.env`:
```env
AD_STUB=true
```

Или разово через переменную окружения:
```bash
AD_STUB=true python3 app.py
```

При `AD_STUB=true` `ad_fetcher.py` полностью делегирует вызовы заглушке — `ldap3` не импортируется, сетевых обращений нет. На странице `/recipients` кнопка «Загрузить из AD» вернёт фиктивных тим-лидов из `STUB_LEADS`.

**Настройка тестовых данных** — редактировать список `STUB_LEADS` в `ad_fetcher_stub.py`:
```python
STUB_LEADS = [
    {"name": "Иванов Иван Иванович", "email": "i.ivanov@company.ru", "department": "Отдел разработки"},
    ...
]
```

Поле `department` должно совпадать со значениями в столбце `COL_DEPT` Excel-файла — иначе сопоставление адресатов с подразделениями работать не будет.
